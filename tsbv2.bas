...
1052  DIM F%(1,6),BD$,I,I1,DT%(3),DC(11),TR$(13),P(1),SC%(8,1,3),SE%(1,4,3),AD%(2,1)
...
28210  MMOB 1,XM(2)+16,YM(2)+32:MOB SET 1,6,1,0,0,0
28211  MMOB 2,XM(2)+16,YM(2)+64:MOB SET 2,6,1,0,0,0
28212  MMOB 3,XM(2)+16,YM(2)+96:MOB SET 3,6,1,0,0,0
28213  MMOB 4,XM(2)+16,YM(2)+128:MOB SET 4,6,1,0,0,0

28214  FORX=0TO3:PB(X,0)=50000:PB(X,1)=0:PB(X,2)=0:NEXT

28220  KEYGET I$:I=PEEK(512)
...
28230  IF I$="Q" THEN PB=0:MOGUY:GOTO 28220
28231  IF I$="W" THEN PB=1:MOGUY:GOTO 28220
28232  IF I$="E" THEN PB=2:MOGUY:GOTO 28220
28233  IF I$="R" THEN PB=3:MOGUY:GOTO 28220
28234  IF I$="X" THEN 28990
28250  GOTO 28220
...
28890  PRINT AT(BY+BH,BX+1) "ASTE";
...
28905  PB(PB,2)=ABS(PB(PB,2)-1)

28910  PB(PB,0)=PB(PB,0)+1000:PB(PB,1)=PB(PB,1)+1
28920  MMOB PB+1,XM(2)+16+PB(PB,1),YM(2)+32*(PB+1)
28925  MOB SET PB+1,6+PB(PB,2),1,0,0,0
...
# TL:TEXT LENGTH
#58252  TX=PEEK(F4+3):TV=TC+TX*3:TL=PEEK(TV):Q=TC+D!PEEK(TV+1)
#58253  Z=DISPLAY+(BY+PEEK(F4+1)*40)+BX+PEEK(F4+2)+39
#58255  D!POKE $5A,Q: D!POKE $58,Z: POKE 781,1: POKE 782,TL
#58260  SYS $A3EC
