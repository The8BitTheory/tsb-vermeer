...
# INIT LV%(Y,X) TO -1
1090  FOR Y=0 TO 3:FOR X = 0 TO 14:LV%(X,Y,2)=-1:NEXT:NEXT
...
9160  IF LV%(TV,CP,0)=0 THEN IF LV%(TV,CP,1)=0 THEN TX$="EINE RNTE":GOTO 9170
9162  TX$="RNTE:"

#     SHOW HARVESTED QUANTITIES SINCE LAST VISIT
9163  Y=BY+5
9164  IF LV%(TV,CP,0)>0 THEN USE AT(Y,BX+3) CF$(3),LV%(TV,CP,0):Y=Y+1
9166  IF LV%(TV,CP,1)>0 THEN USE AT(Y,BX+3) CF$(3),LV%(TV,CP,1)

9170  PRINT AT(BY+4,BX+1) TX$;
...
22840    LV%(CT,PO,PS)=LV%(CT,PO,PS)+FK*WK

#        BOOK PRODUCTION TO STORAGE
22845    SC%(CT,PS,PO)=SC%(CT,PS,PO)+FK*WK
