...
1102  DIM M$(24),F%(1,6),BD$,I,I1,DT(2),DC(11),TR$(19),P(1),SC%(14,1,3),SE%(1,4,3)
1104  DIM LV%(14,3,2),WK%(14,3,1),PC(14,3,1)
...
9184  PRINT AT(BY+8,BX+10) STR$(WK%(PD(CP,0),CP,0))

9186  PRINT AT(BY+9,BX+1) "OHN    :"
9188  PRINT AT(BY+9,BX+10) WK%(PD(CP,0),CP,1)""

9190  PRINT AT(BY+10,BX+1) "OSTEN  :" TX$(PD(CP,0),4)
...
20195  IF QC<>2 THEN PRINT AT(BY+3,BX+5) "LLES";
...
22255  TV=VAL(TX$(PD(CP,0),4))
22260  IF TV>0 THEN PD(CP,1)=PD(CP,1)-TV
...
22810    PL$=PL$(PX):TV=FN PL(9):WK=WK%(PD(CP,0),CP,0)
...
22900    TV=WK%(PD(CP,0),CP,0)*WK%(PD(CP,0),CP,1)
22910    TX$(PD(CP,0),4)=STR$(VAL(TX$(PD(CP,0),4))+TV)
...
24512   QC=2:MV=500-WK%(PD(CP,0),CP,0)
24514   OSELQUA
24516   WK%(PD(CP,0),CP,0)=WK%(PD(CP,0),CP,0)+RV
24517   WK%(PD(CP,0),CP,1)=1
24518  DONE

#      FIRE
24520  IF ID=1 THEN DO
24525   QC=2:MV=WK%(PD(CP,0),CP,0)
24530   OSELQUA
24535   WK%(PD(CP,0),CP,0)=WK%(PD(CP,0),CP,0)-RV
24540  DONE
...
24565   WK%(PD(CP,0),CP,1)=WK%(PD(CP,0),CP,1)+RV
24570  DONE

#      WOO AWAY WORKERS
24580  IF ID=3 THEN DO
24585   QC=2:MV=500-WK%(PD(CP,0),CP,0)
24590   OSELQUA
24595   WK%(PD(CP,0),CP,0)=WK%(PD(CP,0),CP,0)+RV
24600  DONE
