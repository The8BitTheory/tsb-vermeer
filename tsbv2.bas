...
# $7E00 - $7FFF: NATIVE MEMORY EXPANSION ASSEMBLY ROUTINES (DMACOPY, ETC)
# $C000 - : SPRITES.
# $C3C0 - : DMA WORKING MEMORY FOR BASIC  ROUTINES
# $C64D - $C74D: DMA WORKING MEMORY FOR FRAMES.BIN
# $CA00 - $CA79: MEMORY.BIN.  ROUTINE MANAGER. PERMANENT
# $CA80 - $CBFF:  ROUTINES: AUCTION,PLANTATION,SPRITES
# $E800: CHARSET
...
10 TT=D!PEEK($2B):VT=D!PEEK($2D):BL=VT-TT:BB=1:ME=PEEK(1020):MD=$7E00
11 GAMSAVCHK
...
20 POKE56,122:COLOUR0,0,1:CA=$E800:FA=$C64D:NL=$C3C0:N4=NL+4:FM=$0400
...
6933  IF ME=1 THEN MEMDEF 4,NL,GS(1)*4,0,0,0:MEMLOAD
6934  IF ME=2 THEN SYS MD+9,4,GS(1)*4,5,NL,0
6935  IF ME=3 THEN SYS MD+15,4,GS(1)*4,NL
...
6941  ML=OC+OC+2
# THE REU-ADDRESS USUALLY NEEDS TO ADD RA%(0,0), BUT THAT'S ZERO ANYWAYS
6942  IF ME=1 THEN MEMDEF ML,N4,D!PEEK(NL),0,0,0:MEMLOAD:END PROC

6943  IF ME=2 THEN SYS MD+9,ML,D!PEEK(NL),5,N4,0

6944  IF ME=3 THEN SYS MD+15,ML,D!PEEK(NL),N4
...
17110  IF ID=2 THEN WIPEPD:GS(1)=8:ID=0:CALL CHOICEMENU
17120 END PROC
...
17584  IF NOT OK THEN WIPEPD:GS(1)=8:ID=0:CALL CHOICEMENU
...
17610  SYS $CA00,2,TX,TY:OK=PEEK($CA84)>4:T(8)=PEEK($CA84)
...
60010  IF ME=1 THEN MEMDEF 256,$C400,$7F00,0:MEMLOAD:END PROC
60015  IF ME=2 THEN SYS MD+9,256,$7F00,5,$C400,0
60020  IF ME=3 THEN SYS MD+15,256,$C400,$7F00
60030 END PROC
...
60055  IF ME=1 THEN MEMDEF BL,TT,$8000,0:MEMSAVE
60060  RCOMP MEMDEF 256,$C400,$7F00,0:MEMSAVE:END PROC

60065  IF ME=2 THEN SYS MD+9,BL,TT,0,$8000,5
60070  RCOMP SYS MD+9,256,$C400,0,$7F00,5

60075  IF ME=3 THEN SYS MD+9,BL,TT,$8000
60080  RCOMP SYS MD+9,256,$C400,$7F00
