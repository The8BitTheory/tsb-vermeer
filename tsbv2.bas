...
# LV%(CITY-ID,PLAYER-ID): LAST VISIT
# LV%(CITY-ID,PLAYER-ID): LAST HARVEST (HARVEST SINCE LAST VISIT)

1011  DIM M$(20),F%(1,6),BD$,I,I1,DT(2),DC(11),TR$(19),P(1),SC%(14,1,3),SE%(1,4,3)
1012  DIM LV%(14,3),LH%(14,3)
...
# INIT LV%(Y,X) TO -1
1090  FOR Y=0 TO 3:FOR X = 0 TO 14:LV%(X,Y)=-1:NEXT:NEXT
...
9110  BX=15:BY=5:BW=24:BH=4:FRAME
9120  PRINT AT(BY+1,BX+1) TT$(VAL(TX$(PD(CP,0),1)));
9125  USE AT(BY+1,BX+10) CF$(1),T(0)
9130  PRINT AT(BY+2,BX+1) TT$(VAL(TX$(PD(CP,0),2)));
9135  USE AT(BY+2,BX+10) CF$(1),T(1)


9150  BX=1:BY=12:BW=24:BH=11:FRAME

#     LV%() RECORDS ONLY FOR PLANTATIONS. SO, CITY-INDEX IS 6 LESS
9152  IF LV%(TV,CP)>=0 THEN TX$="ETZTER ESUCH VOR":ELSE TX$="RSTER ESUCH"
9154  PRINT AT(BY+1,BX+1) TX$;

9155  RCOMP PRINT AT(BY+2,BX+2) STR$(LV%(PD(CP,0)-6,CP))+" AGEN";

9160  IF LH%(TV,CP)>0 THEN TX$="RNTE:":ELSE TX$="EINE RNTE"
9162  PRINT AT(BY+4,BX+1) TX$;
...
#      SET LAST VISIT IN THIS CITY OF THIS PLAYER TO ZERO DAYS (IF PLANTATION)
10010  IF PD(CP,0)>=6 THEN LV%(PD(CP,0)-6,CP)=0
...
12245  IF PD(CP,0)>=6 THEN LV%(PD(CP,0)-6,CP)=0
...
22643    TV=LV%(X,Y)
22646    IF TV>=0 THEN LV%(X,Y)=TV+1
22649   NEXT:NEXT
