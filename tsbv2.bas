...
1007  DEF FN NL(ZZ)=PEEK(NL+PEEK(NL+GS(1)+GS(1)+GS(1))+ZZ+ZZ)
#     NAVIGATION TARGET-INDEX
1008  DEF FN NT(ZZ)=PEEK(NL+PEEK(NL+GS(1)+GS(1)+GS(1))+ZZ+ZZ+1)

#     NAVIGATION LABEL WIDTH
1009  DEF FN NW(ZZ)=PEEK(NL+GS(1)+GS(1)+GS(1)+1)
#     NAVIGATION LABEL COUNT (NR OF MENU ENTRIES)
1010  DEF FN NC(ZZ)=PEEK(NL+GS(1)+GS(1)+GS(1)+2)
...
17590 PROC DLGOKCANCEL

17592  BX=27:BY=19:BW=12:BH=4:FRAME
17594  PRINT AT(BY+1,BX+1) "EST]TIGEN";
17596  PRINT AT(BY+2,BX+1) "BBRECHEN";
17598  ID=0:TAG
...
# REMOVE PLANTATION FROM PLAYER AND CITY, UPDATE LIST OF AVAILABLE PLANTATIONS
17900 PROC LPLASELL
...
#      ASSIGN PLANTATION TO CITY
17910  PC$(TV)=PC$(TV)+CHR$(PI)

#      ASSIGN PLANTATION TO PLAYER
17920  PP$(CP)=PP$(CP)+CHR$(PI)
...
18010  ISELECTPLA:IF PL<0 THEN 18080
...
#18195  PRINT AT(0,0) "LFINDPLA PL:"PL:DO NULL

#18195  IF PL>=0 THEN TRACE ON:TX$=TX$+" - "+STR$(PL):BOXWITHTEXT:DO NULL
...
18300 PROC ISELECTPLA

18304  SX=24+2*8+12*8+4:SY=50+6*8+9*8+4
18305  RLOCMOB 0,SX,SY,0,1:MOBON 0
18306  PL=-1

18310  TQ=0:REPEAT

18312   KEYGET I$:I=PEEK(512):R=0

# UP
18313   IF I=145 THEN SY=SY-8:R=1:IF SY<YM(0) THEN SY=YM(0)+4:R=2
18314   ON R GOTO 18330,18390

# DOWN
18315   IF I=17 THEN SY=SY+8:R=1:IF SY>YM(1)+16 THEN SY=YM(1)+12:R=2
18316   ON R GOTO 18330,18390

# LEFT
18317   IF I=157 THEN SX=SX-8:R=1:IF SX<XM(0) THEN SX=XM(0)+4:R=2
18318   ON R GOTO 18330,18390

# RIGHT
18319   IF I=29 THEN SX=SX+8:R=1:IF SX>XM(1)+16 THEN SX=XM(1)+12:R=2
18320   ON R GOTO 18330,18390

#ENTER
18322   IF I=13 THEN LFINDPLA:TQ=PL>=0:R=ABS(TQ)+1
18327   ON R GOTO 18390,18390

# EXIT
18328   IF I$="" THEN TQ=-1:GOTO 18390

18330   RLOCMOB 0,SX,SY,0,1
...
#58487  IF GS(1)=13 THEN IPLASELL:END PROC
