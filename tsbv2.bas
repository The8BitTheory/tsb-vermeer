...
# 15 IFE=0THENE=1:PRINT"ADE EXTKONSTANTEN":LOAD"TEXTCONSTS",USE,0,TC - NO MORE
...
#72 RSTER ESUCH
#73 AGEN
9152  IF LV%(CT,CP,2)>=0 THEN TX$=TT$(71):ELSE TX$=TT$(72)
9154  PRINT AT(BY+1,BX+1) TX$;

9155  RCOMP PRINT AT(BY+2,BX+2) STR$(DT%(3)-LV%(CT,CP,2))+" "TT$(73);

#     CHECK HARVEST SINCE LAST VISIT
9160  IF LV%(CT,CP,0)=0 THEN IF LV%(CT,CP,1)=0 THEN TX$=TT$(70):GOTO 9180
9161  TX$=TT$(69)
...
9184  PRINT AT(BY+8,BX+11) STR$(WK%(CT,CP,0));
...
#      OTEL
12020  AT(TX$,TT$(23)):BOXWITHTEXT:AT(TX$,TT$(23))
...
# UTACHTEN
14720  IF ID=1 THEN TX$="AS GEHT NOCH NICHT":BOXWITHTEXT:OGALSEL:END PROC

# ERKAUFEN
14730  IF ID=2 THEN TX$="AS GEHT NOCH NICHT":BOXWITHTEXT:OGALSEL:END PROC
...
24010  GS(1)=20
24020  LCHDR
24030  AT(TX$,TT$(39)):BOXWITHTEXT:AT(TX$,TT$(39))
24040  CHOICEMENU
...
58255  TX=PEEK(F4+3)
58259  Z=DISPLAY+((BY+PEEK(F4+1))*40)+BX+PEEK(F4+2)

58260 PROC CPRX
58265  TV=TC+TX*3:TL=PEEK(TV):Q=TC+D!PEEK(TV+1)
58275  D!POKE $5A,Q: D!POKE $58,Z: POKE 781,1: POKE 782,TL
58280  SYS $A3EC
