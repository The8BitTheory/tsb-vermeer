...
1012  DIM M$(14),F%(1,6),BD$,I,I1,DT(2),DC(11),TR$(19),P(1),SC(14,1,3)
...
#      ORWARD ONTRACTS
20040  BX=1:BY=13:BW=38:BH=5:FRAME
20042  PRINT AT(BY+1,BX+1) "ERMINGESCH]FTE";
20044  PRINT AT(BY+2,BX+1) "AFFEE";
20045  PRINT AT(BY+2,BX+8) "";
20046  PRINT AT(BY+2,BX+12) "12.10.1922";
20047  USE AT(BY+2,BX+23) "###/",0
20048  USE AT(BY+2,BX+27) "###T",123
20049  USE AT(BY+2,BX+31) CF$(4),777

20050  PRINT AT(BY+3,BX+1) "EE";
20052  PRINT AT(BY+3,BX+8) "ON";
20053  PRINT AT(BY+3,BX+12) "1.1.1921";
20054  USE AT(BY+3,BX+23) "###/",0
20056  USE AT(BY+3,BX+27) "###T",246
20057  USE AT(BY+3,BX+31) CF$(4),888
...
20115  T6=ID:OTRANSPROD


20129 END PROC

# SHOW PRODUCE SELECTION FOR TRANSPORT
20130 PROC OTRANSPROD
...
# UP
20231   IF I=145 THEN DW=-1:I(IP)=I(IP)+1:IF I(IP)>9 THEN I(IP)=0
20232   IF DW THEN 20280

# DOWN
20233   IF I=17 THEN DW=-1:I(IP)=I(IP)-1:IF I(IP)<0 THEN I(IP)=9
20234   IF DW THEN 20280

# LEFT
20235   IF I=157 THEN DW=-1:INSEL:IP=IP-1:IF IP<0 THEN IP=0
20236   IF DW THEN INSEL:GOTO 20285

# RIGHT
20237   IF I=29 THEN DW=-1:INSEL:IP=IP+1:IF IP>3 THEN IP=3
20238   IF DW THEN INSEL:GOTO 20285

# RETURN
20240   IF I=13 THEN TQ=-1


# PRINT CURRENT INPUT VALUE
20280   PRINT AT(BY+3,BX+1+IP) ""MID$(STR$(I(IP)),2)"";

20285  UNTIL TQ=-1

20287  BOOKTRANSPORT

20288  GS(1)=9:OWARCHOICE

20289 END PROC

20290 PROC INSEL
20292  INV BY+3,BX+1+IP,1,1
20299 END PROC
...
20310  TV=I(0)*1000+I(1)*100+I(2)*10+I(3)
20320  IF TV=0 THEN END PROC

20322  T3=FNLB(TV):T4=FNHB(TV)

# PRODUCE
20325  T2=VAL(TX$(PD(CP,0),ID+1))

#FNTX GETS FIRST VALUE OF T$
20330  T1=FN TX(0):T$=MID$(T$,2)

#ADD TRANSPORT-ID TO PLAYER'S TRANSPORTS
20340  PT$(CP)=PT$(CP)+CHR$(T1)

20350  T$(T1)=CHR$(T5)+CHR$(CP)+CHR$(T2)+CHR$(T3)+CHR$(T4)+CHR$(ID)
...
22630   FOR PX=0 TO 9
22632    IF LEN(T$(PX))=0 THEN 22639

#        TV:TRANSPORT DURATION LEFT IN DAYS
22634    TV=ASC(LEFT$(T$(PX),1))-1
#        TRANSPORT ARRIVED: BOOK TO STORAGE. ELSE:UPDATE TRANSPORT DURATION
22636    IF TV=0 THEN LARRTRA:ELSE INST(CHR$(TV),T$(PX),1)
...
58485  IF GS(1)=9 THEN IWAR:END PROC
58486  IF GS(1)<12 THEN HANDLECHOICE:END PROC
...
58496  IF GS(1)=16 THEN ITRANSPROD:END PROC
