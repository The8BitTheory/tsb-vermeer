...
# $7C00 (31744): FRAMEDATA - 512 BYTES (408 WITH 33 FRAME DEFS)
...
#100 MEMDEF 1024,$7C00,0,0,0,0:MEMSAVE
...
6901  FR=1:FRN:PRINT AT(BY+1,BX+1) "PIELER"CP"IN "TX$(PD(CP,0),0);

# NZEIGE ARVERMOEGEN
#6903  BX=27:BY=1:BW=12:BH=3:FRAME
6903  FR=2:FRN:USE AT(BY+1,BX+1) CF$(0),PD(CP,1);
...
8464  TS=TI:REPEAT:UNTIL TI-TS>5
8466  MOB SET PB+1,13,1,0,0,0
8468  TS=TI:REPEAT:UNTIL TI-TS>5
...
#14819  IF I$="P" THEN MEMDEF 1000,52224,0,0,0,0:MEMSAVE:MEMDEF 1000,55296,1000,0,128,0:MEMSAVE:GOTO 14816
#14820  IF I$="G" THEN MEMDEF 1000,52224,0,0,0,0:MEMLOAD:MEMDEF 1000,55296,1000,0,128,0:MEMLOAD::GOTO 14816
...
14899 PRINT AT(0,18) D!PEEK(51)-D!PEEK(49):END PROC
...
58201  F2=FA+FR+FR:F4=FA+D!PEEK(F2):BY=PEEK(F4)
58204  BX=PEEK(F4+1):BW=PEEK(F4+2):BH=PEEK(F4+3):FRAME
58205  F4=F4+4
58208  BA=PEEK(F4)
...
58275  TX=TC+TX*3:TL=PEEK(TX):POKEMA,TL

58280  D!POKE$5A,TC+D!PEEK(TX+1):D!POKE$58,MP:POKE781,1:POKE782,TL:SYS $A3EC
#58280  MEMDEFTL,MP,D!PEEK(TV+1),0:MEMLOAD
