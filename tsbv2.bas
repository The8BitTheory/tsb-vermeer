...
# $0400  (1024): TEXT CONSTANTS - 1024 BYTES (870 BYTES CURRENTLY WITH 78 ENTRIES)
# $7C00 (31232): NAVLABELS - 285 BYTES WITH 32 NAV DEFS
# FOLLOWING    : FRAMEDATA - 585 BYTES WITH 36 FRAME DEFS

# $C000 - 49152: SPRITES (896 BYTES) -> 50048 ($C380)
#                1 MORE SPRITE FITS IN HERE
# $C3E7 - END OF USABLE MEMORY
# $C64D - 50765: 256 BYTES AVAILABLE -> $C74D 51021
# $E800 - 59392: CHARSET

# ZEROPAGE: 45/55 REU VAR COPY
#           49/51 FREE MEM DISPLAY

#MEMDEF VL,VA,VA,VB:MEMLOAD:
#4 POKE646,1:PRINT "GAME RV:"RV:PRINT "VA:"D!PEEK($2D)",VE:"D!PEEK($37):DO NULL
5 IF RV THEN POKE $C62C,0:GOTO 115

10 POKE56,124:COLOUR0,0,1:CA=$E800:NL=$7C00:TC=$0400:FA=D!PEEK($FB)
#11 VA=D!PEEK($2D):VE=D!PEEK($37):VL=VE-VA:VB=1

20 PRINT PEEK($C62C)
#21 IFA=0THENA=1:MEM:PRINT"ADE EICHENSATZ":LOAD"CHARSET.V2",USE,0,CA
#22 IFB=0THENB=1:PRINT"ADE PRITES":LOAD"SPRITES",USE,0,$C000
#12 IFB=0THENB=1:PRINT"ADE UKTION":LOAD"WISIM",USE,0,$C000
#23 IFC=0THENC=1:PRINT"ADE AVLABELS":LOAD"NAVLABELS",USE,0,NL
#24 IFD=0THEND=1:FA=D!PEEK($AE):D!POKE$FB,FA:PRINT"ADE AHMEN":LOAD"FRAMES",USE,0,FA
#25 IFE=0THENE=1:PRINT"ADE EXTKONSTANTEN":LOAD"TEXTCONSTS",USE,0,TC
#28 IF PEEK(828)<>$AD THEN PRINT"ADE AUSTREIBER":LOAD"MAUS.DRV",USE,0,$0334

#49 POKE 646,0:TX$="G10:"+CHR$(13)
#50 FORI=1TOLEN(TX$):POKE630+I,ASC(MID$(TX$,I,1)):NEXT:POKE198,LEN(TX$)
#51 D!20-99
#52 DATA 82,213,58,13

100 MEMDEF 1024,TC,0,0,0,0:MEMSAVE
105 IF FA=0 THEN FA=D!PEEK($FB)
110 INIT:STARTGAME
115 SCR:MENUE
...
1000 PROC INIT
1001  X=RND(0):POKE 646,1:KEY OFF
1002  CHECK
...
1007  DEF FN NL(ZZ)=PEEK(NL+D!PEEK(NL+GS(1)*4)+ZZ+ZZ)
#     NAVIGATION TARGET-INDEX
1008  DEF FN NT(ZZ)=PEEK(NL+D!PEEK(NL+GS(1)*4)+ZZ+ZZ+1)

#     NAVIGATION LABEL WIDTH
1009  DEF FN NW(ZZ)=PEEK(NL+GS(1)*4+2)
#     NAVIGATION LABEL COUNT (NR OF MENU ENTRIES)
1010  DEF FN NC(ZZ)=PEEK(NL+GS(1)*4+3)
...
#     FILL THE MENU
6914  FOR I1=0 TO OC:CY=I1+1:TX=FN NL(I1):CPX:NEXT
...
6940 PROC BWTL
6942  BX=1:BY=6:BW=20:BH=5:FRAME
6944  CENTER AT(BY+2,BX)TX$,BW
6949 END PROC
...
8208  PRINT AT(BY+1,BX+1) "UKTIONSTERMINE:";
...
8950  PB%(PB)=PB%(PB)+5:SA=TI:REM SV=60
...
14040  ID=0:BX=BX+12:BY=BY+1:TAG
...
14360  POKE NL+GS(1)*4+3,LEN(AC$(CP,AC))
...
22620   IF DT%(0)=1 THEN IF DT%(1)=1 THEN OTOM:FSB:OTRIPS
...
#####################
28000 PROC OTOM
#28005  D!POKE $FB,VA:D!POKE $FD,VL
#28010  POKE646,0:PRINT"LO"CHR$(34)"NY"CHR$(34)":RUN:"
#28011  POKE198,0:POKE 631,19:POKE 632,13
#28013  POKE 633,13:POKE 198,3
28010  LOAD"NY",USE,0
28014  END

###############
# TEST LOADING OTHER PROGRAM FILE WHILE KEEPING VARIABLES
###############
#29000 PROC PLYR
# 
#29010  TX$="LOADING PLAYER":BWTL
#29020  MEMDEF VL,VA,VA,VB:MEMSAVE

#29030  LOAD "PLAYER",USE,0

#29990  MEMDEF VL,VA,VA,VB:MEMLOAD
...
58410 PROC TAG
58415  INV BY+ID+1,BX+1,FN NW(.),1
58420 END PROC
...
59090 MP$="                                        "
59091 MA=.*LEN(MP$)+D!PEEK(71)
59092 MP=D!PEEK(MA+1)
