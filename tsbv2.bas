...
1007  DEF FN NL(ZZ)=PEEK(NL+PEEK(NL+GS(1)+GS(1)+GS(1))+ZZ)

1008  DEF FN NW(ZZ)=PEEK(NL+GS(1)+GS(1)+GS(1)+1)
1009  DEF FN NC(ZZ)=PEEK(NL+GS(1)+GS(1)+GS(1)+2)
...
1102  DIM M$(24),F%(1,6),BD$,I,I1,DT(2),DC(11),TR$(13),P(1),SC%(8,1,3),SE%(1,4,3)
1104  DIM LV%(8,3,2),WK%(8,3,1),PC(8,3,1)
1106  F=11:FS=12:FB=6:IL=10:ID=0:MI=0:CP=0
1108  T$="":FORX=0TO9:T$=T$+CHR$(X):NEXT
...
# RC:ROOM COUNT
1200  RC=20:DIM RO%(RC,6)
#     NAVIGATION TARGETS
1202  RO%(0,0)=3:RO%(0,1)=10:RO%(0,2)=4:RO%(0,3)=6:RO%(0,4)=7:RO%(0,5)=17
1204  RO%(1,0)=3:RO%(1,1)=5 :RO%(1,2)=4:RO%(1,3)=6:RO%(1,4)=7:RO%(1,5)=17
1206  RO%(2,0)=3:RO%(2,1)=8 :RO%(2,2)=20:RO%(2,3)=4:RO%(2,4)=6:RO%(2,5)=9:RO%(2,6)=17
1208  RO%(3,0)=128:RO%(3,1)=-1
#1025  RO%(4,0)=128:RO%(4,1)=-1
1210  RO%(5,0)=1:REM RO%(5,1)=-1
1212  RO%(6,0)=128:REM RO%(6,1)=-1
1214  RO%(7,0)=128:REM RO%(7,1)=-1

1216  RO%(8,0)=12:RO%(8,1)=13:RO%(8,2)=14:RO%(8,3)=2

1218  RO%(9,0)=2:RO%(9,1)=-1
#1031  RO%(10,0)=18:RO%(10,1)=19:RO%(10,2)=20:RO%(10,3)=128
1220  RO%(11,0)=2:RO%(11,1)=-1
1222  RO%(12,0)=15:RO%(12,1)=15:RO%(12,2)=8
1224  RO%(13,0)=0:RO%(13,1)=-1
1226  RO%(14,0)=0:RO%(14,1)=-1
1228  RO%(17,0)=128:RO%(17,1)=-1
...
1700 DIM PL$(179),PP$(3),PC$(8),CF$(4),PR(179),T$(9),PT$(3)
...
9155  RCOMP PRINT AT(BY+2,BX+2) STR$(LV%(CT,CP,2))+" AGEN";
...
10011   FOR X=0TO2:LV%(CT,CP,X)=0:NEXT
...
12245  IF PD(CP,0)>=6 THEN DO
#       SET LAST VISIT TO ZERO
12247   FORX=0TO2:LV%(PD(CP,0)-6,CP,X)=0:NEXT
#       SET PENDING WAGES TO ZERO
12248   PC(PD(CP,0)-6,CP,0)=0
12249  DONE
...
18004  SX=24+2*8+12*8+4:SY=50+6*8+9*8+4
18005  RLOCMOB 0,SX,SY,0,1:MOBON 0

18010  TQ=0:REPEAT

18012   KEYGET I$:I=PEEK(512):R=0

# UP
18013   IF I=145 THEN SY=SY-8:R=1:IF SY<YM(0) THEN SY=YM(0)+4:R=2
18014   ON R GOTO 18030,18090

# DOWN
18015   IF I=17 THEN SY=SY+8:R=1:IF SY>YM(1)+16 THEN SY=YM(1)+12:R=2
18016   ON R GOTO 18030,18090

# LEFT
18017   IF I=157 THEN SX=SX-8:R=1:IF SX<XM(0) THEN SX=XM(0)+4:R=2
18018   ON R GOTO 18030,18090

# RIGHT
18019   IF I=29 THEN SX=SX+8:R=1:IF SX>XM(1)+16 THEN SX=XM(1)+12:R=2
18020   ON R GOTO 18030,18090

#ENTER
18022   IF I=13 THEN LFINDPLA:IF PL>=0 THEN TQ=-1:GOTO 18090

# EXIT
18024   IF I$="" THEN TQ=-1:GOTO 18090

18030   RLOCMOB 0,SX,SY,0,1

18090  UNTIL TQ

18095  MOBOFF 0:ID=0:GS(1)=8:SCREEN
...
18101  CT=PD(CP,0)-6:PL=-1:PX=1

#TX/TY: TILE X AND Y POSITIONS. CALCULATED FROM SX AND SY
18102  TX=(SX-XM(0)-4)/8:TY=(SY-YM(0)-4)/8
18104  TX$="TX"+STR$(TX)+",TY:"+STR$(TY)
18105  IF LEN(PC$(CT))=0 THEN 18299

18109  REPEAT
18110   TV$=MID$(PC$(CT),PX,1)
18114   PL$=PL$(ASC(TV$))
18115   PX=PX+1
...
18117   T(3)=FN PL(4)
#       Y-COORD
18118   T(6)=FN PL(7)-2:IF T(6)<0 THEN T(6)=0
#       X-COORD
18119   T(7)=FN PL(8)-2

#18120  TX$=TX$+",W="+STR$(T(2))+",H="+STR$(T(3))+",Y="+STR$(T(6))+",X="+STR$(T(7))

18130   IF TX<T(7) THEN 18190
18131   IF TX>=T(7)+T(2) THEN 18190
18132   IF TY<T(6) THEN 18190
18133   IF TY>=T(6)+T(3) THEN 18190
18134   PL=ASC(TV$)

18140   PX=LEN(PC$(CT))+1

18190  UNTIL PX > LEN(PC$(CT))

18191  IF PL<0 THEN END PROC

18195  TX$=TX$+" - "+STR$(PL):BOXWITHTEXT:DO NULL
...
# QC=2: SELECTIGN QUANTITY FOR HIRING WORKERS (NO "LLES")
...
21086   USE AT(BY+1+PX,BX+16) CF$(4),PX*233;
21089  NEXT
...
#       UPDATE CALENDAR (IF MONTH HAS CHANGED: TV=-1 -> SHOW REPORT)
22610   DT(0)=DT(0)+1
22612   IF DT(0)>DC(DT(1)-1) THEN DT(1)=DT(1)+1:DT(0)=1:TV=-1
...
22615   IF TV THEN OMON:FILL 0,0,40,19,0,15:OTRIPS
...
#      WOO AWAY WORKERS
24580  IF ID=3 THEN DO
24585   QC=2:MV=500-WK%(CT,CP,0)
24590   OSELQUA
24595   WK%(CT,CP,0)=WK%(CT,CP,0)+RV
24600  DONE

24605  IF ID<4 THEN FILL 19,1,18,6,0,15:GS(1)=20:CHOICEMENU:END PROC
...
25030  PRINT AT(BY+1,BX+1) "ONATSBERICHT: "TT$(DT(1)+4)DT(2)

#      PRINT STOCK NAMES
25040  FOR X=0 TO 3
25050   PRINT AT(BY+3+X,BX+1) TT$(17+X)
25060   REM PRINT PRICE
25070  NEXT

#      PRINT PRODUCE NAMES
25100  FOR X=0 TO 3
25110   PRINT AT(BY+3+X,BX+20) TT$(X)
25120   REM PRINT PRICE
25130  NEXT
...
# IOVE SHOULD GO INTO HANDLECHOICE DIRECTLY
