...
14 IFD=0THEND=1:PRINT"ADE AHMEN":LOAD"FRAMES",USE,0,FA
...
# PRINT AT 1MHZ: 214 JIFFIES/MEGA
...
22620   IF DT%(0)=1 THEN IF DT%(1)=1 THEN IF DT%(2)>1918 THEN OTOM:FILL 0,0,40,25,0,15:OTRIPS
...
23148   PRINT AT(BY+1+X,BX+1) T1"AGE, ";
23149   TX=T3:CY=1+X:CX=11:CPX
23150   PRINT AT(BY+1+X,BX+18) ", "(T4+T5*256)"T,"TX$(T6,0);
...
58225  IF BA=1 THEN CPR:GOTO58205
#58230  IF BA=2 THEN CENTER AT(BY+PEEK(F4+1),BX+PEEK(F4+2)) TT$(PEEK(F4+4)),PEEK(F4+3);:F4=F4+5:GOTO 58208
...
58300 PROC FRAME
58312  INSERT BD$,BY,BX,BW,BH,FB:FILL BY+1,BX+1,BW-2,BH-2,32,1
58320  FCOL BY+BH,BX+1,BW,1,11:FCOL BY+1,BX+BW,1,BH,11
58330 END PROC

# AHMENZEICHEN
58350 DATA 111,183,112,180,32,170,108,187,188
