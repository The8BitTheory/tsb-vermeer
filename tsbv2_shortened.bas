...
# $C000 - 49152: SPRITES (512 BYTES) -> 49663 ($C1FF)
# $C200 - 49664: NAVLABELS (512 BYTES) -> WILL GO AWAY (OR CHANGE)
# $C800 - 51200: SPACE FOR PETSCII ART -> 2KB -> 53247 ($CFFF)
# $E800 - 59392: CHARSET

100 POKE56,120:COLOUR0,0,1:CA=$E800:NL=$C200
...
# AN$(PLAYER-ID,ARTCAT-ID):ART NEW. IDS OF ARTWORK THAT ARE NEW
...
1103 AC$(0,0)=CHR$(0)+CHR$(1)+CHR$(2):AN$(0,0)=CHR$(0)+CHR$(1)+CHR$(2)
...
6820  IF GS(1)=0 THEN OHU:END PROC
6821  IF GS(1)=1 THEN OHUB:END PROC
6822  IF GS(1)=2 THEN OHUBP:END PROC
6823  IF GS(1)=3 THEN OT:END PROC
6825  IF GS(1)=4 THEN OH:END PROC
6827  IF GS(1)=5 THEN OG:END PROC
6828  IF GS(1)=6 THEN OB:END PROC
6829  IF GS(1)=7 THEN O:END PROC
6830  IF GS(1)=8 THEN OP:END PROC
6831  IF GS(1)=9 THEN OW:END PROC
6832  IF GS(1)=10 THEN OM:END PROC


# UY PLANTATION
6834  IF GS(1)=12 THEN OPL:END PROC
# ELL PLANTATION
6835  IF GS(1)=13 THEN OPLAS:END PROC
# MANAGE PLANTATION
6836  IF GS(1)=14 THEN OPLA:END PROC

# PLANT PLANTATION
#6837  IF GS(1)=15 THEN OPLAPLANT


6837  IF GS(1)=17 THEN OO:END PROC

6838  IF GS(1)=18 THEN OSE:END PROC
6839  IF GS(1)=19 THEN OMA:END PROC

6840  IF GS(1)=20 THEN OWO:END PROC

6841  IF GS(1)=21 THEN OS:END PROC

# MANAGE WORKERS AT PLANTATION (WORKER ASSIGNMENT)
6842  IF GS(1)=22 THEN OWOR:END PROC

#     BANK - REEDEREIEN
6843  IF GS(1)=23 THEN CHO:END PROC
#     BANK - DOLLAR
6844  IF GS(1)=24 THEN CHO:END PROC
#     BANK - LOANS
6845  IF GS(1)=25 THEN CHO:END PROC

#     REEDEREIEN SELECTIONS
6846  IF GS(1)=26 THEN QC=3:OBANR:END PROC
6847  IF GS(1)=27 THEN QC=4:OBANR:END PROC

6848  IF GS(1)=28 THEN QC=5:OBAN:END PROC
6849  IF GS(1)=29 THEN QC=6:OBAN:END PROC

6850  IF GS(1)=30 THEN QC=7:OBA:END PROC
6851  IF GS(1)=31 THEN QC=8:OBA:END PROC

6852  IF GS(1)=32 THEN OGA:END PROC
...
6901  BX=1:BY=1:BW=26:BH=3:FR
6902  PRINT AT(BY+1,BX+1) "PIELER"CP"IN "TX$(PD(CP,0),0);

# NZEIGE ARVERMOEGEN
6903  BX=27:BY=1:BW=12:BH=3:FR
...
6911  OC=FN NC(.)
6912  BX=27:BY=20-OC:BW=FN NW(.)+2:BH=3+OC:FR
...
9110  BX=15:BY=5:BW=24:BH=5:FR
9115  PRINT AT(BY+1,BX+1) "RBEITSKR]FTE";
9117  USE AT(BY+1,BX+17) CF$(6),0;
9120  PRINT AT(BY+2,BX+1) "LANTAGEN "TT$(VAL(TX$(PD(CP,0),1)));
9125  USE AT(BY+2,BX+17) CF$(1),T(0);
9130  PRINT AT(BY+3,BX+1) "LANTAGEN "TT$(VAL(TX$(PD(CP,0),2)));
9135  USE AT(BY+3,BX+17) CF$(1),T(1);


9150  BX=1:BY=12:BW=22:BH=12:FR
...
10005  IF ID>=15 OR ID=PD(CP,0) THEN ID=0:F:S:END PROC


10009  CT=PD(CP,0)-6
10010  IF CT>=0 THEN LL
...
10025  BX=1:BY=21:BW=24:BH=3:FR
10030  PRINT AT(BY+1,BX+1) "EISEKOSTEN";
10031  DM=413
10032  USE AT(BY+1,BX+13) CF$(0),DM:LDE
10034  PAUSE 3
...
10810  BX=1:BY=1:BW=24:BH=18:FR

#      ANZEIGE SPIELERNAME UND AKTUELLE STADT
10820  BX=1:BY=21:BW=24:BH=3:FR
10825  PRINT AT(BY+1,BX+1) "PIELER"CP"IN " TX$(PD(CP,0),0);

#      AUSWAHL DER STADT
10830  BX=26:BY=1:BW=13:BH=18:FR
...
12022  BX=27:BY=20-OC:BW=12:BH=3+OC:FR
...
12210  IF ID>1 THEN F:ID=0:S:END PROC
...
13100  BX=1:BY=3:BW=38:BH=10:FR
13110  FOR X=0 TO 4
...
13200  BX=1:BY=15:BW=22:BH=6:FR
...
14010  LD
14015  BX=1:BY=4:BW=38:BH=13:FR
...
14290  IF ID>7 THEN ID=0:F:S:END PROC
...
14310  BX=1:BY=4:BW=38:BH=13:FR
...
14510  IF ID<LEN(AC$(CP,AC)) THEN AS=ID:OGAL:END PROC
...
#14605  SCRLD DEF $CC,4,0,2:SCRLD 1,USE,2,IN$:SCRLD RESTORE
...
# NSEHEN
14710  IF ID=0 THEN OGALV:END PROC

# UTACHTEN
14720  IF ID=1 THEN TX$="AS GEHT NOCH NICHT":B:OGAL:END PROC

# ERKAUFEN
14730  IF ID=2 THEN TX$="AS GEHT NOCH NICHT":B:OGAL:END PROC

#BX=27:BY=20-OC:BW=FN NW(.)+2:BH=3+OC:FRAME
14790  ID=AC:FILL 4,0,40,20,0,15:OGA
...
#      DRAW GRASS
16035  BX=1:BY=5:BW=25:BH=19:FR
...
17102  IF ID<2 THEN OPLAP:END PROC

17110  IF ID=2 THEN W:GS(1)=8:ID=0:CHO:END PROC
17120 END PROC
...
17503  BX=27:BY=9:BW=12:BH=10:FR
...
17584  IF NOT OK THEN W:GS(1)=8:ID=0:CHO:END PROC
...
17710  IF ID=0 THEN LPL:LD:GOTO 17714
...
18010  ISEL:IF PL<0 THEN GS(1)=8:S:END PROC

18015  BX=27:BY=9:BW=12:BH=10:FR
...
#ENTER
18270   IF I=13 THEN LF:TQ=PL>=0:R=ABS(TQ)+1
...
18410  BX=27:BY=5:BW=12:BH=11:FR
...
19020  GS(1)=8:CHO:IF PL>=0 THEN T
...
20064  BX=19:BY=19:BW=20:BH=5:FR
...
20110  IF ID>1 THEN ID=0:F:S:END PROC
...
20134  BX=19:BY=19:BW=12:BH=5:FR
#      CHOICE MENU FOR PRODUCE SELECTION
20136  OPR
...
20176  BX=1:BY=19:BW=17:BH=5:FR
...
# LEFT
20242   IF I=157 THEN DW=-1:INS:IP=IP-1:IF IP<0 THEN IP=0
20244   IF DW THEN INS:GOTO 20285

# RIGHT
20246   IF I=29 THEN DW=-1:INS:IP=IP+1:IF IP>QM THEN IP=QM
20248   IF DW THEN INS:GOTO 20285
...
21010  BX=1:BY=5:BW=38:BH=11:FR

#      DRAW QUANTITIES IN STOCK, AND PRICE
21070  BX=1:BY=17:BW=24:BH=6:FR
...
21110  IF ID=0 THEN QC=0:OSE:END PROC
...
# T3: OMARPRO,IMARPRO
# T4/T5: VOLUME
21115  IF ID=1 THEN QC=1:OSE:END PROC

# ERMINE
21120  IF ID=2 THEN OMA:END PROC

# USGANG
21130  IF ID=3 THEN ID=0:F:S:END PROC
...
22360  BX=1:BY=19:BW=38:BH=5:FR
...
22615   IF DT%(0)=1 THEN OMO:FILL 0,0,40,19,0,15:OTR
...
23035  TX$=PT$(CP):TV$=CHR$(PX):LR:PT$(CP)=RV$
...
23062  IF TV=1 THEN RV$=MID$(TX$,2):END PROC

#      LAST ENTRY, USE LIST EXCEPT LAST ENTRY.
23064  IF TV=T1 THEN RV$=LEFT$(TX$,T1-1):END PROC

#      SOMEWHERE IN THE MIDDLE, USE BEFORE AND AFTER
23066  RV$=LEFT$(TX$,TV-1)+MID$(TX$,TV+1)
...
#      RANSPORTS
23140  BX=1:BY=12:BW=38:BH=5:FR
...
23146   TV=ASC(MID$(PT$(CP),X,1)):TD$=T$(TV):L
...
23340    IF TV=0 THEN LAR:GOTO 23350
...
23410  BX=30:BY=17:BW=9:BH=7:FR
...
23581  IF QC=1 THEN T2=ID:LB:GOTO 23590
...
23595  TX$="QC="+STR$(QC)+",PRD:"+STR$(ID)+",VOL:"+STR$(RV):B
23596  KEYGET I$

23598  GS(1)=10:ID=0:S
...
24605  IF ID<4 THEN FILL 19,1,18,6,0,15:GS(1)=20:ID=0:CHO:END PROC

24610  IF ID=4 THEN ID=0:F:S
...
25010  BX=1:BY=1:BW=38:BH=14:FR
...
25302  OC=FN NC(.)
25304  BX=27:BY=20-OC:BW=FN NW(.)+2:BH=3+OC:FR
...
58442     IF GS(1)=99 THEN PRINT AT(0,38) "J";:IJ

58444     R=0:KEYGET I$:I=PEEK(512)
58446     OC=FN NC(.)

58448     IF I=145 THEN T:ID=ID-1:R=-1:IF ID<0 THEN ID=OC
58450     IF I=17 THEN T:ID=ID+1:R=-1:IF ID>OC THEN ID=0

58452     IF I$="F" THEN POKE0,65
58454     IF I$="S" THEN POKE0,64

58456     IF I=13 THEN H
...
58473  IF GS(1)=3 THEN IT:END PROC
58474  IF GS(1)=9 THEN IW:END PROC
58475  IF GS(1)=10 THEN IM:END PROC
58476  IF GS(1)=11 THEN IGA:END PROC
58477  IF GS(1)=4 THEN IH:END PROC
58478  IF GS(1)=5 THEN I:END PROC
58479  IF GS(1)<12 THEN HA:END PROC
...
58480  IF GS(1)=12 THEN IP:END PROC
58481  IF GS(1)=13 THEN IPL:END PROC
58482  IF GS(1)=14 THEN IPLAMAN:END PROC
58483  IF GS(1)=15 THEN IPLA:END PROC
58484  IF GS(1)=16 THEN ITR:END PROC

58485  IF GS(1)=17 THEN HA:END PROC

58486  IF GS(1)=18 THEN ISE:END PROC
58487  IF GS(1)=19 THEN IMA:END PROC
58488  IF GS(1)=20 THEN IWO:END PROC

# GS(1)=21 IS ISELQUA, WHICH IS DIRECTLY CALLED FROM OSELQUA

58489  IF GS(1)=23 THEN HA:END PROC
58490  IF GS(1)=24 THEN HA:END PROC
58491  IF GS(1)=25 THEN HA:END PROC

58492  IF GS(1)=26 THEN QC=3:IB:END PROC
58493  IF GS(1)=27 THEN QC=4:IB:END PROC
...
58498 IF GS(1)=32 THEN IG:END PROC
...
58505  TV=FN NT(ID)
58510  IF TV<128 THEN GS(1)=TV: ELSE F
58515  ID=0
58520  S
